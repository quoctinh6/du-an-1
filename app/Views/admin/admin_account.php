        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle admin-table">
            ...
            <tbody>
              ...
              <?php if (!empty($users)): ?>
                <?php foreach ($users as $user): ?>
                  ...
                  <tr>
                    <td>
                      <img
                        src="https://ui-avatars.com/api/?name=<?= urlencode($user['name']) ?>&background=<?= $bg_color ?>&color=fff"
                    ...
                    <td>
                      <span
                        class="badge <?= $is_active ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger' ?>">
                        <?= $is_active ? 'Hoạt động' : 'Đã khóa' ?>
                      </span>
                    </td>
                    <td class="text-end">
                      <a href="<?= BASE_URL ?>index.php/admin/user?id=<?= $user['id'] ?>"
                      ...
                      <?php if (!$is_self): // Không cho phép khóa chính mình ?>
                        <?php if ($is_active): ?>
                          <a href="<?= BASE_URL ?>index.php/admin/updateStatus?id=<?= $user['id'] ?>&status=0"
                            class="btn btn-sm btn-outline-danger" title="Khóa tài khoản"
                            onclick="return confirm('Bạn có chắc chắn muốn khóa tài khoản <?= htmlspecialchars($user['name']) ?> không?');">
                            <i class="bi bi-lock"></i>
                          </a>
                        <?php else: ?>
                          <a href="<?= BASE_URL ?>index.php/admin/updateStatus?id=<?= $user['id'] ?>&status=1"
                            class="btn btn-sm btn-outline-success" title="Mở khóa"
                            onclick="return confirm('Bạn có chắc chắn muốn mở khóa tài khoản <?= htmlspecialchars($user['name']) ?> không?');">
                      ...
                      <?php if ($is_admin): // Nếu đang là Admin, hiện nút chuyển thành Client ?>
                        <a href="<?= BASE_URL ?>index.php/admin/updateRole?id=<?= $user['id'] ?>&role=customer"
                          class="btn btn-sm btn-outline-secondary" title="Hạ cấp quyền"
                          onclick="return confirm('Bạn có chắc chắn muốn hạ cấp tài khoản <?= htmlspecialchars($user['name']) ?> xuống Client không?');">
                          <i class="bi bi-person-dash"></i>
                        </a>
                      <?php elseif (!$is_admin): // Nếu đang là Client, hiện nút chuyển thành Admin ?>
                        <a href="<?= BASE_URL ?>index.php/admin/updateRole?id=<?= $user['id'] ?>&role=admin"
                          class="btn btn-sm btn-outline-secondary" title="Thăng cấp quyền"
                          onclick="return confirm('Bạn có chắc chắn muốn thăng cấp tài khoản <?= htmlspecialchars($user['name']) ?> lên Admin không?');">
              ...
              <?php else: ?>
                <tr>
                  <td colspan="6" class="text-center py-4 text-muted">Không tìm thấy tài khoản nào phù hợp với điều kiện
                    tìm kiếm/lọc.</td>
                </tr>
              <?php endif; ?>
    ...
    <nav aria-label="Account pagination" class="d-flex justify-content-center mt-4">
      <ul class="pagination">
        <li class="page-item <?= $currentPage <= 1 ? 'disabled' : '' ?>">
          <a class="page-link"
            href="<?= BASE_URL ?>index.php/admin/account?page=<?= max(1, $currentPage - 1) ?>&role=<?= $currentRole ?>&status=<?= $currentStatus ?>&search=<?= $currentSearch ?>">Trước</a>
        </li>

        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
          <li class="page-item <?= $i == $currentPage ? 'active' : '' ?>">
            <a class="page-link"
              href="<?= BASE_URL ?>index.php/admin/account?page=<?= $i ?>&role=<?= $currentRole ?>&status=<?= $currentStatus ?>&search=<?= $currentSearch ?>"><?= $i ?></a>
          </li>
        <?php endfor; ?>

        <li class="page-item <?= $currentPage >= $totalPages ? 'disabled' : '' ?>">
          <a class="page-link"
            href="<?= BASE_URL ?>index.php/admin/account?page=<?= min($totalPages, $currentPage + 1) ?>&role=<?= $currentRole ?>&status=<?= $currentStatus ?>&search=<?= $currentSearch ?>">Sau</a>
        </li>
      </ul>
    </nav>
    </div>
        ...
        <form action="<?= BASE_URL ?>index.php/admin/addUser" method="POST">
          ...
          <div class="mb-3">
            ...
            <input type="password" class="form-control" name="password" id="userPassword"
              placeholder="Đặt mật khẩu ban đầu" required minlength="6">
          </div>
          <div class="mb-3">
